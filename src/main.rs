...
use opencl3::memory::{Buffer, CL_MEM_READ_ONLY, CL_MEM_WRITE_ONLY, CL_MEM_SVM_ATOMICS};
...
kernel void compare_kernel(global const int* source, global int* counts)
{
    ...
    {
        atomic_fetch_add((atomic_int*)&counts[i],1);
    }
}
"#;
...
kernel void assign_kernel(global const int* source, global const int* counts, global int* results)
{
    ...
    {
        expected = -1;
        atomic_compare_exchange_strong((atomic_int*)&results[counts[my_id]+i],&expected,source[my_id]);
        i++;
...
fn main() -> Result<()>
{
    ...
    io::stdin().read_line(&mut max_sort_value).expect("Failed to read line");
    ...
    io::stdin().read_line(&mut max_array_size).expect("Failed to read line");
    ...
    //Build Kernels
    let compare_program = Program::create_and_build_from_source(&context, COMPARE_KERNEL, "-cl-std=CL2.0")
    .expect("Compare Kernel Failed to Build");
    let compare_kernel = Kernel::create(&compare_program, COMPARE_KERNEL_NAME).expect("Compare Kernel Create Failed");

    let assign_program = Program::create_and_build_from_source(&context, ASSIGN_KERNEL, "-cl-std=CL2.0").expect("Assign Kernel Failed to Build");
    let assign_kernel = Kernel::create(&assign_program, ASSIGN_KERNEL_NAME).expect("Assign Kernel Create Failed");
    ...
    let mut input_buffer = Buffer::<cl_int>::create(&context, CL_MEM_READ_ONLY, max_array_size, ptr::null_mut())?;
    let mut count_buffer = Buffer::<cl_int>::create(&context,  CL_MEM_SVM_ATOMICS, max_array_size, ptr::null_mut())?;
    let mut result_buffer = Buffer::<cl_int>::create(&context, CL_MEM_WRITE_ONLY | CL_MEM_SVM_ATOMICS, max_array_size, ptr::null_mut())?;

    let input_write_event = queue.enqueue_write_buffer(&mut input_buffer, CL_BLOCKING, 0, &input, &[])?;
    let count_write_event = queue.enqueue_write_buffer(&mut count_buffer, CL_BLOCKING, 0, &vec![0; max_array_size], &[])?;
    let result_write_event = queue.enqueue_write_buffer(&mut result_buffer, CL_BLOCKING, 0, &vec![-1;max_array_size],&[])?;
    ...
    let read_event = queue.enqueue_read_buffer(&result_buffer, CL_BLOCKING, 0, &mut result, &events)?;
    ...
    print!("\n");
    println!("MergeSort execution duration (ns): {}\n", elapsed.unwrap_or_default().as_nanos());
